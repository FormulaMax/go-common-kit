...
func TestNewPriorityQueue(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "无边界",
			q:        NewPriorityQueue[int](0, compare()),
			capacity: 0,
			data:     data,
		...
		{
			name:     "有边界 ",
			q:        NewPriorityQueue[int](len(data), compare()),
			capacity: len(data),
			data:     data,
			expected: []int{1, 2, 3, 4, 5, 6},
		},
...
func TestPriorityQueue_Shrink(t *testing.T) {
	var compare go_common_kit.Comparator[int] = func(a, b int) int {
		if a < b {
			return -1
		}
		if a == b {
...
func priorityQueueOf(capacity int, data []int, compare go_common_kit.Comparator[int]) *PriorityQueue[int] {
...
func compare() go_common_kit.Comparator[int] {
